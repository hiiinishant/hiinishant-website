          ...
          {/* Instagram handle badge */}
            ...
            <div className="absolute bottom-3 left-3 z-20 flex items-center gap-1.5 bg-black/65 backdrop-blur-md rounded-full px-2.5 py-1 border border-white/10">
              <div className="w-4 h-4 rounded-full overflow-hidden relative">
                <Image src="/profile.jpg" alt="Nishant Kumar — hiiinishant, Founder of 2 AM Study" fill sizes="16px" className="object-cover object-top" />
              </div>
  ...
  return (
    <section id="latest-updates" className="py-10 lg:py-16 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative z-10">
        ...
        <div
          ref={sectionRef}
          className={`transition-all duration-1000 ${sectionVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
            }`}
        >
          {/* Giant title */}
          ...
          <div
            ref={statsRef}
            className={`flex items-center justify-center mb-8 px-2 transition-all duration-1000 delay-200 ${statsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
              }`}
          >
            {/* YouTube stat */}
            <div className="flex items-center gap-2.5 sm:gap-3.5">
              <div className="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 shrink-0">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </div>
              <div>
                <p className="text-xl sm:text-2xl font-black text-red-400 leading-none tabular-nums">
                  <AnimatedCounter target={totalVideos} duration={1600} />+
                </p>
                <p className="text-[11px] sm:text-xs text-brand-400 mt-0.5 whitespace-nowrap">
                  <span className="text-white/70 font-medium">Videos</span>
                  <span className="mx-1 opacity-40">·</span>YouTube
                </p>
              </div>
            </div>

            {/* Divider */}
            <div className="w-px h-8 sm:h-10 bg-white/10 mx-3.5 sm:mx-8 shrink-0" />

            {/* Instagram stat */}
            <div className="flex items-center gap-2.5 sm:gap-3.5">
              <div className="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#E1306C]/10 border border-[#E1306C]/20 text-[#E1306C] shrink-0">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
                </svg>
              </div>
              <div>
                <p className="text-xl sm:text-2xl font-black text-[#E1306C] leading-none tabular-nums">
                  <AnimatedCounter target={totalInstagram} duration={1600} />+
                </p>
                <p className="text-[11px] sm:text-xs text-brand-400 mt-0.5 whitespace-nowrap">
                  <span className="text-white/70 font-medium">Posts</span>
                  <span className="mx-1 opacity-40">·</span>Instagram
                </p>
              </div>
            </div>
          </div>
          ...
          {/* ── CATEGORY TABS ── */}
          <div className="flex justify-center border-b border-white/8 mb-10 pb-0 px-2">
            <div className="relative inline-flex items-center gap-1 p-1 sm:p-1.5 rounded-2xl glass-strong overflow-x-auto max-w-full scrollbar-hide mb-[-1px]">
              {tabStyle.width > 0 && (
                <span
                  className="absolute top-1 bottom-1 sm:top-1.5 sm:bottom-1.5 bg-accent rounded-xl transition-all duration-300 ease-out shadow-[0_0_20px_rgba(245,158,11,0.3)] pointer-events-none"
                  style={{
              ...
              {categories.map((cat, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`relative z-10 flex items-center gap-1.5 sm:gap-2 px-3.5 sm:px-5 py-2 sm:py-3 rounded-xl text-xs sm:text-sm font-medium whitespace-nowrap transition-colors duration-300 ${isActive ? "text-black font-semibold" : "text-brand-300 hover:text-white"
                      }`}
                  >
        ...
        {/* ── CARDS GRID ── */}
        <div
          className={`transition-all duration-300 ${isTransitioning ? "opacity-0 scale-[0.98] translate-y-2" : "opacity-100 scale-100 translate-y-0"
            }`}
        >
