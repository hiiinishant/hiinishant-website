...
const vlogs_1 = __importDefault(require("./routes/vlogs"));
...
app.use('/api/vlogs', vlogs_1.default);
...
const convoParticipantsCache = new Map();
...
io.on('connection', (socket) => {
    ...
    socket.on('register', (userId) => {
        ...
        if (db_1.firestore) {
            db_1.firestore.collection('users').doc(userId).set({ lastSeen: null }, { merge: true }).catch(() => { });
        }
    ...
    socket.on('save-call-log', async (data) => {
        ...
        if (!conversationId || !callerId || !calleeId || !db_1.firestore)
            return;
        try {
    ...
    socket.on('send-message', async (data) => {
        ...
        if (!conversationId || !senderId || !db_1.firestore)
            return;
        try {
            const timestamp = new Date();
            const messageData = {
    ...
    socket.on('react-message', async (data) => {
        ...
        if (!conversationId || !messageId || !userId || !reaction || !db_1.firestore)
            return;
        try {
            const msgRef = db_1.firestore
    ...
    socket.on('mark-as-read', async (payload) => {
        ...
        if (!conversationId || !userId || !db_1.firestore)
            return;
        try {
            const messagesRef = db_1.firestore
                .collection('conversations')
                .doc(conversationId)
                .collection('messages');
            ...
            if (snapshot.empty)
                return;
            const batch = db_1.firestore.batch();
            snapshot.docs.forEach((doc) => {
                batch.update(doc.ref, { read: true });
            });
            await batch.commit();
            await db_1.firestore
                .collection('conversations')
                .doc(conversationId)
    ...
    socket.on('disconnect', () => {
        ...
        const timer = setTimeout(() => {
            ...
            if (db_1.firestore) {
                db_1.firestore.collection('users').doc(userId).set({ lastSeen }, { merge: true }).catch(() => { });
            }
